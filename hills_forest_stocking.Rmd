...
The following outlines the data processing and mapping outputs completed in the remapping exercise of the Hills Forest Area. To map potential differences between official data and newly LiDAR-derived attributes, two variables were tested: 'Stocking Density (stem/ha)' & 'Age Class'. Using metre-resolution DEM and DSM raster datasets from the LINZ website, a stem map was derived using variable window function. From this layer, a canopy model and stocking density rasters were mapped to compare values with stand records (HILL_0341-009). 'Age Class' was categorized roughly using available data of "Establishment Year" and then compared visually with classified canopy height. 
